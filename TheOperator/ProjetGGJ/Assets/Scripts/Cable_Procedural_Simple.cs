...
public class Cable_Procedural_Simple : MonoBehaviour {
    ...
    public void setEndPoint(GameObject end)
    {
        cable_end = end;
        endPointTransform = end.transform;
    }

    public void deleteEnd()
    {
        cable_end.GetComponent<Cable_Simple_End>().deleteEnd();
    }
    ...
    void OnMouseDown()
    {
        GameObject manager = GameObject.Find("CableManager");
        ...
        if (is_set == true)
        {
            ...
            for (var i = 0; i < tab.Length; ++i)
            {
                if (tab[i].GetComponent<BoxScript>().boxInfos.idName == holeId)
                {
                    ...
                    {
                        Debug.Log("DELETING BECAUSE CLICKED");
